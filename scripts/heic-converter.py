...
def heic_to_jpg(heic_file, jpg_file):
  """
  Convert a HEIC file to JPG format using ImageMagick's `convert` command.

  Args:
    heic_file (str): The path to the HEIC file.
    jpg_file (str): The path to save the converted JPG file.

  Raises:
    subprocess.CalledProcessError: If the conversion process fails.
    Exception: If any other error occurs during the conversion.
